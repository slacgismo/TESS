"""Renamed interval to meter_interval table, removed pv state out/state in, added pv_interval table

Revision ID: 843e1aeb407c
Revises: 3e7ea9bdacbb
Create Date: 2020-06-08 12:17:42.170163
...
def upgrade():
    ...
    op.create_table('pv_intervals',
    sa.Column('pv_interval_id', sa.Integer(), autoincrement=True, nullable=False),
    ...
    sa.ForeignKeyConstraint(['pv_id'], ['pvs.pv_id'], ),
    sa.ForeignKeyConstraint(['rate_id'], ['rates.rate_id'], ),
    sa.PrimaryKeyConstraint('pv_interval_id')
    )
...
def downgrade():
    ...
    sa.Column('meter_id', mysql.VARCHAR(length=64), nullable=False),
    sa.Column('rate_id', mysql.INTEGER(), autoincrement=False, nullable=False),
    sa.Column('status', mysql.ENUM('one'), nullable=False),
