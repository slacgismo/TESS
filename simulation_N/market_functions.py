...
def create_market_noEIM(df_forecast,df_prices,p_max,prec,price_intervals,dt_sim_time):
    ...
    #forward prices
        ...
        minutes = int(price_intervals*interval/60)
        df = df_forecast.loc[dt_sim_time:(dt_sim_time+datetime.timedelta(minutes=minutes))].copy()
        df['mc'] = df['coincident_peak_forecasted']*coincident_peak_rate
        df['mc'] += (1 - df['coincident_peak_forecasted'])*fixed_procurement_cost
        mean_p = df['mc'].sum()
        var_p = df['mc'].var()
    #forward prices
    elif ref_price == 'none':
